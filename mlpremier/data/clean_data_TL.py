...
def create_player_meta_csv(season:str, 
                           save_dir:str=os.path.join('clean_data_TL', 'meta')):
    """ 
...
def reorder_and_limit_gwdata_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    desired_cols = ['name', 'position', 'team', 'opponent_team', 'minutes', 
                    ...
                    'influence', 'creativity', 'threat', 'was_home', 'kickoff_time',
                    'total_points']
...
def clean_player_data(season: str, 
                    save_dir: str = os.path.join('clean_data_TL'),
                    verbose:bool = False):
    """
    ...
    RAW_DATA_DIR = 'raw_data'
    PLAYER_META_CSV_PATH = os.path.join('clean_data_TL', 'meta', f'player_meta_{season}.csv')
...
def main():
    ...
    if args.organize:
        organize_data_by_pos('clean_data_TL', verbose=True)
